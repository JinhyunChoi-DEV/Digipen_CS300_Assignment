...
Mesh::Mesh(std::string name, std::vector<glm::vec3> vertices, std::vector<glm::vec3> vertexNormals,
		...
		: name(name), positions(vertices), vertexNormals(vertexNormals), faceNormal(faceNormal), indices(indices), type(type)
{
	shader = GRAPHIC->GetShader("Default");
...
Mesh::Mesh(std::string name, std::vector<glm::vec3> vertices, DrawType type)
	: name(name), positions(vertices), type(type)
{
